...
var RTCMultiConnection = function(roomid, forceOptions) {
	...
	console.log("IN RTCMulticonnection FUNCTION");
	...
	console.log("connection.enableLogs(1)",connection.enableLogs);

        if (connection.enableLogs) {
            if (connection.socketURL == '/') {
                console.info('socket.io url is1: ', location.origin + '/');
            } else {
                console.info('socket.io url is2: ', connection.socketURL);
            }
        }

        try {
		console.log("TRY connection.socket with params");
		connection.socket = null;
		console.log('socket.io url is3: ', connection.socketURL);
		...
		// console.log("AFTER: connection.socket =",connection.socket);
        } catch (e) {
		console.log("Error catch in Try connection socket");
		...
		console.log("onMessageEvent function");
		...
		stream = connection.streamEvents[message.message.streamid];
	...
	// console.log("BEFORE connection.socket.on");
        connection.socket.on('connect', function() {
	    	console.log("RTCMULTICONNECTION: IN connection.socket.on('connect.....)");
            if (alreadyConnected) {
                return;
            }
            alreadyConnected = true;

	    console.log("connection enablelogs(2)",connection.enableLogs);
            if (connection.enableLogs) {
		// console.log('socket.io connection is opened.');
            }

            setTimeout(function() {
		console.log("RTCMULTICONNECTION: BEFORE socket.emit('extra-data-updated')");
		...
		console.log("connection.socket.onError");
	...
	console.log("KIND: ",kind);
		...
		console.log("RETURN stream.getTracks");
    ...
    function PeerInitiator(config) {
		...
		console.log("1BEFORE NEW RTCPEERconnection");
		   ...
		   console.log("2BEFORE NEW RTCPEERconnection");
                    peer = new RTCPeerConnection(params);
                } catch (e) {
		    console.log("3BEFORE NEW RTCPEERconnection");
	...
	console.log("IN FUNCTION CONNECTION");
		...
		console.log("typeof SocketConnection =:",SocketConnection);
			...
			console.log("new SocketConnection function");
                if (connectCallback) {
                    connectCallback(connection.socket);
                }
            });
		...
		console.log("Before connection.join1");
		...
		console.log("On connection.join1, remoteuserid=",remoteUserId);
        ...
        function openRoom(callback) {
            if (connection.enableLogs) {
                console.log('Sending open-room signal to connection.socket.emit');
            }
            ...
            connection.socket.emit('open-room', {
                ...
                if (isRoomOpened === true) {
                    if (connection.enableLogs) {
                        console.log('RTCMULTICONNECTION isRoomOpened: ', isRoomOpened, ' roomid: ', connection.sessionid);
                    }
	...
	console.log("connection.userid random:",connection.userid);
		...
		console.log("XXXXXXXXXXXXXXXXXaddstream");
		...
		console.log("IN connection.replacetrack");
		console.log("IN connection.replacetrack.session", session);
            session = session || {};
	    ...
	    console.log("Before connection.join3");
